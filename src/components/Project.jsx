...
const customClassName =
  'bg-nice-green w-[60px] py-2 rounded-md font-bold hover:bg-nice-nude hover:text-nice-green focus:bg-nice-nude focus:text-nice-green';

function Project() {
  const { projectId } = useParams();

  const project = projectsData.find(
    (projectData) => projectData.id === parseInt(projectId, 10)
  );
  ...
  return (
    <article className='mx-auto px-4 py-2 flex flex-col md:justify-center items-center md:items-start md:flex-row md:gap-4 md:mt-4'>
      <div className='text-center mt-6 md:text-left md:w-5/6'>
        <h1
          className={`${move ? 'translate-x-0' : 'translate-x-24'} transition-transform duration-700 ease-in-out font-amatic text-3xl font-bold mb-6 md:text-6xl`}>
          {title}
        </h1>
        <p>{fullDescription}</p>
